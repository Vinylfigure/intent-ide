...
- [ ] **Rotate the committed `.env` key:** Code is already pushed to private `origin/main`, but a key reached git history before `.gitignore` covered it. The user must still rotate the secret (and ideally scrub history). The repo being private mitigates but does not remove the exposure.
...
* **Secret committed to git history (still unrotated):** A key was committed in `.env` before `.gitignore` covered it. The code is now pushed to the private GitHub repo `Vinylfigure/intent-ide` `main` (3 commits), so the key exists in remote history. The repo being private mitigates exposure, but the user must still rotate the key and ideally scrub history. Do not treat this as resolved until rotation is confirmed.
